  ...
  function makeSet() {
    ...
    set.goTo = symbol => {
      ...
      g.assignHandles(
        Handle.closure(
          handles
            .filter(item => item.hasNext() && item.getCurrentSymbol() === symbol)
            .map(item => Handle.getNext(item))
        )
      );
      return g;
    };
    set.getEdges = () => {
      const found = [];
      return handles
        .filter(item => item.hasNext())
        .map(item => item.getCurrentSymbol())
        .filter(symbol => {
          if (found[symbol]) {
            return false;
          }
    ...
    set.equals = s => {
      ...
      return (
        handles.every(a => sh.some(b => Handle.equals(a, b))) &&
        sh.every(b => handles.some(a => Handle.equals(a, b)))
      );
    };
    Object.freeze(set);
    return set;
      ...
      cc[i].getHandles().forEach(item => {
        if (!item.hasNext()) {
          if (actionTable[i][item.getEnd()] !== undefined) {
            ConflictRecorder.recordConflict(
              i,
              item.getEnd(),
              {
