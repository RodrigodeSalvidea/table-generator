...
`;
  ...
  function formatGotoTable(cc, rules) {
    ...
    for (let state = 0; state < gotoTable.length; state++) {
      ...
      for (let i = 0; i < nonTerminals.length; i++) {
        gotoTableString +=
          gotoTable[state][nonTerminals[i]] === undefined
            ? 'UNDEFINED'
            : String(gotoTable[state][nonTerminals[i]]);
  ...
  function formatRuleSizes(rules) {
    return `{${rules
      .getAllRules()
      .map(rule => (rule.getRightSide()[0] === EPSILON ? 0 : rule.getRightSide().length))
      .join(', ')}}`;
  }
  function formatRuleReductions(rules) {
    return `{${rules
      .getAllRules()
      .map(rule => `PARSER_${rule.getLeftSide()}`)
      .join(', ')}}`;
  }
...
`;
  ...
  function formatTokenNameTable(rules) {
    return `
char* tokenNameTable[ 2 ][ NUM_TERMINALS > NUM_NONTERMINALS ? NUM_TERMINALS : NUM_NONTERMINALS ] = { 
	{
${rules
  .getTerminals()
  .map(name => `		"${name}"`)
