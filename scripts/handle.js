const Handle = (() => {
  function makeHandle(rule, endSymbol, index) {
    if (rule.getRightSide()[0] === EPSILON) index = 1;
    ...
    function getCurrentSymbol() {
      if (rule.getRightSide()[index] !== undefined) return rule.getRightSide()[index];
      throw new Error(
        'Attempted to get current symbol on a handle where the current symbol is undefined'
      );
    }
    function toString() {
  ...
  function getNext(handle) {
    ...
    if (index === rule.getRightSide().length + 1) {
      throw new Error(
        `Attempted to getNext on a handle with a length of ${rule.getRightSide().length} from index ${index - 1}`
      );
    }
  ...
  function equals(handle1, handle2) {
    return (
      handle1.getRule() === handle2.getRule() &&
